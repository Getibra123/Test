...
public class Setups {
	...
	/**
	 * a normal paddle with rather standard center. the next method returns a point right above this paddle
	 * (to bounce balls on it for instance)
	 */
	public static PaddleState typicalPaddle() {
		Vector padOffset = new Vector(Constants.BLOCK_WIDTH/2,Constants.BLOCK_HEIGHT/2);
		Point padTL =
				new Point(Constants.BLOCK_WIDTH * 5, Constants.BLOCK_HEIGHT * 6 );
		Point padCenter = padTL.plus(padOffset);
		return new NormalPaddleState(padCenter, Constants.TYPICAL_PADDLE_COLORS(), Constants.TYPICAL_PADDLE_COLORS()[0]);
	}
	
	/**
	 * balls with such a position and a down velocity will bounce a standard paddle (see prev method) soon
	 */
	public static Point typicalBouncePosition() {
		Vector padOffset = new Vector(Constants.BLOCK_WIDTH/2,Constants.BLOCK_HEIGHT/2);
		Point padTL =
				new Point(Constants.BLOCK_WIDTH * 5, Constants.BLOCK_HEIGHT * 6 );
		Point padCenter = padTL.plus(padOffset);
		
		Vector bounceOffSet = new Vector( 0 , - Constants.PADDLE_HEIGHT - 1500 );
		return padCenter.plus(bounceOffSet);
	}
